...
3.	找到每个候选框在feature map上的映射patch，将此patch作为每个候选框的卷积特征输入到SPP layer和之后的层  **如何映射？？？？**   
4.	对候选框中提取出的特征，使用分类器判别是否属于一个特定类   
5.	对于属于某一特征的候选框，用回归器进一步调整其位置  
**创新**ROI Pooling，这个网络层可以把不同大小的输入映射到一个固定尺度的特征向量   
...
**创新**加入一个提取边缘的神经网络，也就说找到候选框的工作也交给神经网络来做Region Proposal Network(RPN)   
...
标准RNN前向输出流程，与传统NN相同，多一个隐含层间权重 激活函数一般都为tanh函数，加上偏置  
**RNN的训练方法-BPTT**  
...
需要寻优的参数有三个，分别是U、V、W 其中W和U两个参数的寻优过程需要追溯之前的历史数据，参数V相对简单只需关注目前    
对于求解参数V的偏导数，里面涉及激活函数的中间导，是复合函数求导   
RNN的损失也是会随着时间累加的，所以不能只求t时刻的偏导  
激活函数选择ReLU函数，解决梯度消失与梯度爆炸问题  
