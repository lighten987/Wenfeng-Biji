...
**RNN与因果卷积网络**  
RNN（Recurrent Neural Network）是一类用于处理序列数据的神经网络。后面的数据跟前面的数据有关系。  
在标准的RNN结构中，隐层的神经元之间也是带有权值的。也就是说，随着序列的不断推进，前面的隐层将会影响后面的隐层。  
特点：  
1、权值共享，图中的W(层间权重)全是相同的，U(输入层权重)和V(隐含层输出权重)也一样。   
2、每一个输入值都只与它本身的那条路线建立权连接，不会和别的神经元连接。 
其他结构：多输入多输出(标准结构)，多输入单输出，单输入多输出，Encoder-Decoder模型(MIMO，有个单输出的中间态作为后半部分单输入)  
